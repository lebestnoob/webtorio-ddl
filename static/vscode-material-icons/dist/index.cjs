...
__export(lib_exports, {
  ...
  getIconForDirectoryPath: () => getIconForDirectoryPath,
  getIconForFilePath: () => getIconForFilePath,
  getIconInfo: () => getIconInfo,
  getIconUrlByName: () => getIconUrlByName,
  getIconUrlForFilePath: () => getIconUrlForFilePath,
  isMaterialIconName: () => isMaterialIconName
});
module.exports = __toCommonJS(lib_exports);

// generated/icon-map.json
...
var MATERIAL_ICONS = Object.keys(icon_map_default.iconDefinitions);
...
function getIconNameForFileName(fileName) {
  return icon_map_default.fileNames[fileName] ?? icon_map_default.fileNames[fileName.toLowerCase()] ?? icon_map_default.fileExtensions[getFileSuffix(fileName)] ?? icon_map_default.fileExtensions[getFileExtension(fileName)] ?? (fileName.endsWith(".html") ? "html" : null) ?? (fileName.endsWith(".ts") ? "typescript" : null) ?? (fileName.endsWith(".js") ? "javascript" : null) ?? "file";
}
function getIconNameForDirectoryName(dirName) {
  return icon_map_default.folderNames[dirName] ?? icon_map_default.folderNames[dirName.toLowerCase()] ?? "folder";
}
...
function findKeysByValue(needle, haystack) {
  return Object.entries(haystack).filter(([, value]) => value === needle).map(([key]) => key);
}

// src/lib/functions.ts
function isMaterialIconName(value) {
  return typeof value === "string" && value in icon_map_default.iconDefinitions;
}
function getIconUrlByName(iconName, iconsUrl) {
  return `${iconsUrl}/${iconName.toString()}.svg`;
}
function getIconUrlForFilePath(path, iconsUrl) {
...
function getIconInfo(name) {
  const fileExtensions = findKeysByValue(name, icon_map_default.fileExtensions);
  const fileNames = findKeysByValue(name, icon_map_default.fileNames);
  const folderNames = findKeysByValue(name, icon_map_default.folderNames);
  const folderNamesExpanded = findKeysByValue(
    name,
    icon_map_default.folderNamesExpanded
  );
  return {
    name,
